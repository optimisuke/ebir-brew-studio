...
- **コードの層（1ファイル内で責務を分離）**: ①データ（`BEERS` / バッジ・称号定義 / 定数）→ ②ロジック（`context()` / `evaluateBadges()` / `titleInfo()` / コーデック等、localStorage も DOM も触らない純粋関数）→ ③ストア（`commitRecord()` / `addRecord()` / `addEventRecord()` など localStorage 操作。**DOM は触らず**「増えたバッジ・称号昇格・コンプ達成」を effects として返す）→ ④UI（`render()` / 各シート / トースト / 演出）。ストアが返した effects は UI 側の `reflectEffects()` がトースト・金色演出に反映する（副作用を記録ロジックから分離）
- **ビールの単一情報源**: JS の `BEERS` 配列（`id` / `name` / `style` / `abv` / `ibu` / `img` or `color` / `taste` / `pairing` / `desc` / `active` / `new`）。`active:false` は一覧から外れる（記録には残る）、`new:true` は隠しバッジ「新作ハンター」の対象。`taste`（味わいキーワード文字列）・`pairing`（ペアリング料理の配列）・`desc`（説明文）はビールをタップした記録シート（登録前）に表示される
