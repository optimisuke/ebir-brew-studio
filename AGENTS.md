...
- **ビールの単一情報源**: JS の `BEERS` 配列（`id` / `name` / `style` / `abv` / `ibu` / `img` or `color` / `taste` / `pairing` / `desc` / `active` / `new`）。`active:false` は一覧から外れる（記録と隠しバッジ「伝説の一杯」には残る）、`new:true` は隠しバッジ「新作ハンター」の対象。`taste`（味わいキーワード文字列）・`pairing`（ペアリング料理の配列）・`desc`（説明文）はビールをタップした記録シート（登録前）に表示される
