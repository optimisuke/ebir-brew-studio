...
- **ページ構成（上から）**:
  ...
  4. **バッジ** — `grid-cols-3 sm:grid-cols-4`。獲得は `border-gold ring-gold/40 bg-gold/5`＋右上に金色の✓マーク（`bg-gold text-paper` の丸）、未獲得は `opacity-60`＋アイコン `grayscale`、隠しバッジ未獲得は 🔒＋「???」。獲得条件（`hint`）は常に表示し（隠しバッジは未獲得のあいだ「？？？」で伏せる）、「獲得」の文字は使わない。末尾に店舗向け QR ページへの控えめなリンク
